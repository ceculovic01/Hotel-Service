...
import com.raf.notificationservice.dto.PasswordResetRequestDto;
...
public class EmailListener {
    ...
    public void updatePassword(Message message) throws  JMSException {
        PasswordResetRequestDto passwordResetRequestDto = messageHelper.getMessage(message, PasswordResetRequestDto.class);
        String link = "http://localhost:3000/passwordreset";
        String text = String.format("Da resetujete sifru kliknite na sledeci link %s.", link);
        notificationService.sendSimpleMessage(passwordResetRequestDto.getEmail(), "Password reset", text);
        notificationService.createNotification(passwordResetRequestDto.getEmail(), "passwordReset", text);
    }
