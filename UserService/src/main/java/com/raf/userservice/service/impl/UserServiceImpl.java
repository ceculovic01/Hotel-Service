...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public void passwordResetRequest(PasswordResetRequestDto passwordRequest) {
        Optional<User> user = userRepository.findUserByEmail(passwordRequest.getEmail());
        if(!user.isPresent()){
            throw new NotFoundException("User with given email doesn't exist.");
        }
        jmsTemplate.convertAndSend(updatePasswordDestination, messageHelper.createTextMessage(passwordRequest));

    }

    @Override
    public void passwordReset(TokenRequestDto tokenRequestDto) {
        Optional<User> user = userRepository.findUserByEmail(tokenRequestDto.getEmail());
        if(!user.isPresent()){
            throw new NotFoundException("User with given email doesn't exist.");
        }
        user.get().setPassword(tokenRequestDto.getPassword());
        userRepository.save(user.get());
    }
}
